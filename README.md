...
### Conceptos generales
- SpringBoot: SpringBoot es un framework que nos da la facilidad de crear aplicaciones web basados en anotaciones, con este framework es posible realizar perticiones REST las cuales nos sirven para conectar el front-end y el back-end mediante un API Rest que generamos por nuestra cuenta, el formato de mensajes entre back y front son de tipo Json.
- Maven: Maven nos sirve para manejar el ciclo de vida del proyecto, es decir, con este podemos manejar cada una de las dependencias que necesitara el proyecto, en este caso las dependencias que mas se utilizaron fueron de SpringBoot ya que el poryecto esta basado en este, maven como esta listo para trabajar en red, al ejecutarlo nos permite descargar u obtener las dependencias que necesitamos de manera inmediata solo con la conexion de este a su repositorio en internet.
- Heroku: Heroku es una platafarorma como servicio (PaaS) la cual nos da la posibilidad de subir nuestra aplicacion web a un servidor sin preocuparnos por la cuestiones administrativas del mismo ni las configuraciones, por lo tanto, usar heroku significa solo preocuparnos por el desarrollo de la apliacion web, ya que este servicio nos configura todo lo necesario para que la aplicacion sea desplegada en internet.
- MongoDB: MongoDB es una base de datos NoSQL, es decir una base de datos que es no relacional, la cual nos permite guardar objetos del mismo tipo si seguir una estrucutra dada con es dado en SQL, por lo tanto el manejo de los datos dentro de la base de datos es mucho mas sencillo ya que MongoDB guarda sus datos en este caso como codigos Json basdos en Bson, lo que hace mucho mas sencillo el manejo de los datos. Para este proyecto utilizamos una plataforma de MongoDB Online la cual se llama mlab.com la cual nos ofrece la creacion de registros y conexion a la base de datos mediante una configuracion basica en los archivos del proyecto.
- Bootstrap: Bootstrap es el framework para desarrollo de paginas web front end mas usado en el momento, las ventajas que nos ofrece este framework es que el uso de sus librerias o codigo Open Source nos ayudan a crear sitios web que se adaptan a cualquier tipo de pantalla, es decir sitios web Responsive Design, esto para que la experiencia del usuario con la aplicacion sea agradable y muy poco confusa.
- Axios: Axios es una librebria de JavaScript la cual nos permite consumir un API mediante peticiones REST, con estas peticiones podemos obtener, manejar y enviar codigo entre el API y nuestra aplicacion web, por lo tanto el uso de una libreria como estas nos facilita la obtencion y manejo del API que en este caso es manejada con datos Json.
### Implementacion
